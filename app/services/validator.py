...
def evaluate_thai_id_risk(data: ThaiIDExtraction) -> tuple[list[str], float]:
    ...
    if len(clean_id) != 13 or not clean_id.isdigit():
        flags.append("INVALID_ID_NUMBER_FORMAT: Thai ID must be exactly 13 numeric digits.")
        risk_score += 0.6
    ...
    if data.expiry_date.strip().lower() != "lifetime":
        try:
            ...
            if expiry_dt < current_dt:
                flags.append(f"DOCUMENT_EXPIRED: Identification card expired on {data.expiry_date}")
                risk_score += 0.8
        except ValueError:
            flags.append("DATE_PARSING_ERROR: Expiry date does not match expected YYYY-MM-DD pattern.")
            risk_score += 0.4
...
def evaluate_medical_claim_risk(data: MedicalReceiptExtraction) -> tuple[list[str], float]:
    """
    ...
    high_risk_keywords = ["cosmetic", "plastic surgery", "laser", "whitening", "aesthetic", "ศัลยกรรม", "เลเซอร์"]
    
    for item in data.items:
        desc_lower = item.description.lower()
        if any(keyword in desc_lower for keyword in high_risk_keywords):
            flags.append(f"HIGH_RISK_ITEM_FOUND: Claim item '{item.description}' matches non-covered coverage policies.")
            risk_score += 0.4
