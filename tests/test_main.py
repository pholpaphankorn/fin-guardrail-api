...
class TestMainEndpoints:
    ...
    @patch("app.main.evaluate_blur_dependency")
    async def test_validate_thai_id_unreadable_extraction_failed_case(self, mock_blur_dep, mock_extract):
        ...
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                ...
                payload = response.json()
                assert payload["status"] == "REJECTED"
                assert "UNREADABLE_DOCUMENT" in payload["validation_flags"][0].split(":")[
                    0]
        finally:
    ...
    @patch("app.main.extract_thai_id")
    @patch("app.main.evaluate_blur_dependency")
    async def test_validate_thai_id_approved_happy_case(self, mock_blur_dep, mock_extract, mock_risk):
    ...
    @patch("app.main.evaluate_blur_dependency")
    async def test_validate_medical_receipt_flagged_for_review_edge_case(self, mock_blur_dep, mock_extract, mock_risk):
        ...
        try:
            ...
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                response = await ac.post(
                    ...
                    files={
                        "file": ("receipt.jpg", create_dummy_image_bytes(), "image/jpeg")},
                )
